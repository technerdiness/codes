...
import type {
  ContextoAnswerDateSource,
  ContextoAnswerResult,
  ContextoAnswerSource,
  ContextoRevealTile,
} from "../types";

const USER_AGENT = "Mozilla/5.0 (compatible; RobloxCodesBot/1.0)";
const DEFAULT_SECTION_HEADING = "What is Today's Contexto Answer?";
const ANSWER_SECTION_SELECTOR =
  ".wp-block-beebom-puzzle-features-answer-reveal .answer-reveal__container";
const ANSWER_HEADING_ID = "#h-what-is-today-s-contexto-answer";
...
export const DEFAULT_CONTEXTO_URL = "https://beebom.com/puzzle/contexto-answer-today/";
...
export async function revealContextoAnswer(url: string): Promise<ContextoAnswerResult> {
  ...
  let extractedFrom: ContextoAnswerSource | null = answer ? "answer-reveal:data-answer" : null;
  ...
  if (!answer || !extractedFrom) {
    throw new Error(
      `Could not extract the Contexto answer from ${url} using ${ANSWER_SECTION_SELECTOR} or schema fallbacks.`
    );
  }
...
function extractTiles(
  ...
  return container
    .find(".answer-reveal__tiles-wrapper .answer-reveal__tile")
    .toArray()
    ...
    .filter((tile): tile is ContextoRevealTile => tile !== null);
}
...
function extractFaqAnswerText(graph: SchemaGraphEntry[]): string | null {
  ...
  const targetQuestion = questions.find((question) => {
    ...
    return name.includes("what is today's contexto") || name.includes("contexto answer today");
  });
...
function resolveAnswerDate(input: {
  ...
  if (titleCandidate) {
    return { answerDate: titleCandidate, answerDateSource: "page-title" };
  }

  const ogTitleCandidate = extractIsoDate(input.ogTitle);
  if (ogTitleCandidate) {
    return { answerDate: ogTitleCandidate, answerDateSource: "og-title" };
  }

  const modifiedCandidate = extractIsoDate(input.modifiedAt);
  if (modifiedCandidate) {
    return { answerDate: modifiedCandidate, answerDateSource: "modified-at" };
  }

  const publishedCandidate = extractIsoDate(input.publishedAt);
  if (publishedCandidate) {
    return { answerDate: publishedCandidate, answerDateSource: "published-at" };
  }
